...
本系統採用**絕對觀點**（absolute views），P 矩陣為 one-hot 向量，每行只有一個 1，對應特定股票的預期超額報酬。
...
使用 Anthropic Claude Sonnet 作為量化分析師，生成每月股票的相對績效預測。關鍵設計決策：
...
```python
CONFIDENCE_SCALARS = {
    "high":   0.25,   # Ω = 0.25 × (τ × P × Σ × P')  → 觀點影響力大
    "medium": 1.00,   # Ω = 1.00 × (τ × P × Σ × P')  → 標準影響力
    "low":    4.00,   # Ω = 4.00 × (τ × P × Σ × P')  → 影響力小
}
```
...
Oracle 每月使用最近 **9 個月（~194 個交易日）** 的滾動窗口，確保模型始終反映當前市場環境。
...
| 指標 | Black-Litterman | Markowitz | Equal Weight | SPY（基準） |
|------|:-:|:-:|:-:|:-:|
| **總報酬** | **+58.20%** | +35.18% | +24.16% | +3.03% |
| **年化報酬** | **52.70%** | 44.69% | 38.91% | 15.01% |
| **年化波動率** | 32.08% | 31.27% | 28.90% | 18.01% |
| **Sharpe 比** | **1.518** | 1.302 | 1.208 | 0.612 |
| **Sortino 比** | **2.066** | 1.777 | 1.602 | 0.734 |
| **最大回撤** | -48.75% | -47.24% | -49.38% | -33.72% |
| **Calmar 比** | **1.081** | 0.946 | 0.788 | 0.445 |
| **獲勝率（日）** | 56.14% | 56.06% | 56.22% | 55.53% |
| **平均換手率** | 2.01% | 1.04% | 0.04% | 0% |
| **Beta（vs SPY）** | 0.489 | 0.408 | 0.352 | 1.000 |
| **年化 Alpha** | 86.2% | 72.3% | 62.3% | 0% |

**回測期間**：2021-01-01 至 2024-12-31（約 4 年）

### 12.2 主要發現

1. **Black-Litterman 顯著優於基準**：總報酬 58.20%，為 SPY 的 19 倍，顯示 Mag7 集中策略 + LLM 觀點的有效性。

2. **風險調整報酬最優**：Sharpe 比 1.518 遠超 SPY（0.612），代表每單位風險獲得了更多報酬。

3. **Alpha 顯著**：年化 Alpha 86.2 bps，遠超交易成本（~15 bps），具備可行的超額報酬來源。
